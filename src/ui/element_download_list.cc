...
ElementDownloadList::ElementDownloadList() :
  ...
  m_bindings[KEY_UP]    = m_bindings[control->ui()->get_keymap(RT_KEY_UP)]    = std::bind(&ElementDownloadList::receive_prev, this);
  m_bindings[KEY_DOWN]  = m_bindings[control->ui()->get_keymap(RT_KEY_DOWN)]  = std::bind(&ElementDownloadList::receive_next, this);

  m_bindings[KEY_PPAGE] = m_bindings[control->ui()->get_keymap(RT_KEY_PPAGE)] = [this] { receive_pageprev(); };
  m_bindings[KEY_NPAGE] = m_bindings[control->ui()->get_keymap(RT_KEY_NPAGE)] = [this] { receive_pagenext(); };

  m_bindings[KEY_HOME]  = m_bindings[control->ui()->get_keymap(RT_KEY_HOME)]  = [this] { receive_home(); };
  m_bindings[KEY_END]   = m_bindings[control->ui()->get_keymap(RT_KEY_END)]   = [this] { receive_end(); };
 
  m_bindings[control->ui()->get_keymap(RT_KEY_TOGGLE_LAYOUT)] = std::bind(&ElementDownloadList::toggle_layout, this);
}
